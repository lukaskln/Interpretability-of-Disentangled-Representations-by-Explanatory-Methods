...
class betaTCVAE_CNN(pl.LightningModule):
    def __init__(self,
        ...
        self.enc_conv1 = nn.Conv2d(
            in_channels=1, out_channels=c, kernel_size=4, stride=2, padding=1)  # out: c x 14 x 14
        self.enc_conv2 = nn.Conv2d(
            in_channels=c, out_channels=c*2, kernel_size=4, stride=2, padding=1)  # out: c x 7 x 7
        self.fc_mu = nn.Linear(in_features=c*2*7*7, out_features=latent_dim)
        self.fc_logvar = nn.Linear(
            in_features=c*2*7*7, out_features=latent_dim)
